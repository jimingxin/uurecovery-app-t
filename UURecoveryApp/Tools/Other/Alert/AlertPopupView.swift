...
// 输入完成
typealias AlertInputCompletion = (String, String)->()
...
extension AlertPopupView {
    ...
    /// 弹出姓名和手机输入框
    /// - Parameter show: 回调
    static func showInputView(show: @escaping AlertInputCompletion, close: @escaping AlertPopupCompletion = {} ) {
        let v_input = AlertInputView(show: show, close: close)
        AlertPopupView.showAlertView(contentView: v_input,
                                     container: UIWindow.keyWindow,
                                     isDismissable: false) {
